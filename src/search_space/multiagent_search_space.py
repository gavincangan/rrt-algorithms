...
class MultiAgentSearchSpace(object):
    def __init__(self, dimension_lengths, num_agents=1, O=None):
        """
    ...
    def obstacle_free(self, x):
        ...
        for ix in range(self.num_agents):
            if self.obs.count( x[ ix*self.dimensions : self.dimensions] ) != 0:
                return False
        return True
    ...
    def sample(self):
        ...
        """
        x = np.empty(self.dimensions*self.num_agents, np.float)
        for ix in range(self.dimensions * self.num_agents):
            dim = ix % self.dimensions
            x[ ix ] = random.uniform(self.dimension_lengths[ dim ][0], self.dimension_lengths[ dim ][1])
